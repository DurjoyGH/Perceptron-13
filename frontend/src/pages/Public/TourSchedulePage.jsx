...
const TourSchedulePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      {/* Header */}
      <section className="relative overflow-hidden bg-gradient-to-br from-[#19aaba] via-[#158c99] to-[#116d77] text-white py-20">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-lg md:text-xl text-gray-100 max-w-2xl mx-auto">
              Track every moment of our 9-day journey • December 04-12, 2025
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 max-w-4xl mx-auto">
            <div className="bg-white/10 backdrop-blur-lg rounded-xl p-4 border border-white/20 text-center">
              ...
              <div className="text-xs text-white/80 uppercase font-medium">Total Days</div>
            </div>
            <div className="bg-green-500/20 backdrop-blur-lg rounded-xl p-4 border border-green-400/30 text-center">
              <div className="text-3xl font-bold mb-1">{stats.completed}</div>
              <div className="text-xs text-white/80 uppercase font-medium">Completed</div>
            </div>
            <div className="bg-blue-500/20 backdrop-blur-lg rounded-xl p-4 border border-blue-400/30 text-center">
              <div className="text-3xl font-bold mb-1">{stats.ongoing}</div>
              <div className="text-xs text-white/80 uppercase font-medium">Ongoing</div>
            </div>
            <div className="bg-gray-500/20 backdrop-blur-lg rounded-xl p-4 border border-gray-400/30 text-center">
              <div className="text-3xl font-bold mb-1">{stats.upcoming}</div>
              <div className="text-xs text-white/80 uppercase font-medium">Upcoming</div>
            </div>
            <div className="bg-red-500/20 backdrop-blur-lg rounded-xl p-4 border border-red-400/30 text-center">
              <div className="text-3xl font-bold mb-1">{stats.cancelled}</div>
              <div className="text-xs text-white/80 uppercase font-medium">Cancelled</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 overflow-x-auto py-4 no-scrollbar">
            {[
              { value: 'all', label: 'All Days', icon: <Calendar className="w-4 h-4" /> },
              { value: 'completed', label: 'Completed', icon: <CheckCircle2 className="w-4 h-4" /> },
              { value: 'ongoing', label: 'Ongoing', icon: <PlayCircle className="w-4 h-4" /> },
              { value: 'upcoming', label: 'Upcoming', icon: <Clock3 className="w-4 h-4" /> },
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          {filteredSchedule.length === 0 ? (
            ...
            <div className="relative">
              {/* Vertical Line */}
              <div className="absolute left-8 top-0 bottom-0 w-1 bg-gradient-to-b from-[#19aaba] via-[#158c99] to-[#116d77] rounded-full"></div>
              ...
              <div className="space-y-8">
                {filteredSchedule.map((daySchedule, dayIndex) => {
                  ...
                  return (
                    ...
                    >
                      {/* Day Marker */}
                      <div className="absolute left-4 w-8 h-8 -translate-x-1/2 z-10">
                        <div className={`w-8 h-8 ${statusConfig.dotColor} rounded-full flex items-center justify-center border-4 border-white shadow-lg ${
                      ...
                      <div className="ml-20">
                        <div 
                          className={`bg-white rounded-2xl border-2 ${statusConfig.borderColor} overflow-hidden cursor-pointer hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1`}
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="flex items-center gap-3 mb-3">
                                  ...
                                  <div className={`flex items-center gap-1.5 ${statusConfig.bgColor} ${statusConfig.textColor} px-3 py-1 rounded-full border ${statusConfig.borderColor}`}>
                                    {statusConfig.icon}
                                ...
                                <div className="flex items-center gap-4 text-gray-600">
                                  <div className="flex items-center gap-2">
                                    <Calendar className="w-4 h-4" />
                                    <span className="text-sm font-medium">{daySchedule.date}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <MapPin className="w-4 h-4" />
                                    <span className="text-sm font-medium">{daySchedule.location}</span>
                                  </div>
                                </div>
                              ...
                              <div className="ml-4">
                                <div className={`w-12 h-12 rounded-full ${statusConfig.bgColor} flex items-center justify-center transition-transform ${
                                  expandedDay === daySchedule.day ? 'rotate-180' : ''
                                }`}>
                                  <ChevronDown className={`w-6 h-6 ${statusConfig.textColor}`} />
                                </div>
                              </div>
                            </div>
                          </div>
                                  ...
                                  return (
                                    ...
                                    >
                                      {eventIndex !== daySchedule.events.length - 1 && (
                                        <div className={`absolute left-6 top-16 bottom-0 w-0.5 ${eventStatusConfig.borderColor}`}></div>
                                      )}
                                      
                                      <div className={`flex gap-4 p-4 rounded-xl border-2 ${eventStatusConfig.borderColor} ${eventStatusConfig.bgColor} transition-all duration-300 hover:shadow-lg hover:scale-[1.02]`}>
                                        <div className="flex-shrink-0">
                                          <div className={`w-12 h-12 rounded-xl ${
                                            ...
                                            {event.status === 'completed' ? (
                                              ...
                                              <div className="w-6 h-6">{event.icon}</div>
                                            )}
                                          </div>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                          <div className="flex items-center gap-3 mb-2 flex-wrap">
                                            <span className={`text-sm font-bold ${eventStatusConfig.textColor} ${eventStatusConfig.bgColor} px-3 py-1 rounded-full`}>
                                              {event.time}
                                            </span>
                                            <div className={`flex items-center gap-1.5 ${eventStatusConfig.bgColor} ${eventStatusConfig.textColor} px-2.5 py-1 rounded-full text-xs font-bold uppercase border ${eventStatusConfig.borderColor}`}>
                                              {eventStatusConfig.icon}
                                              <span>{eventStatusConfig.label}</span>
                                            </div>
                                            {event.status === 'ongoing' && (
                                              <span className="px-2.5 py-1 bg-blue-500 text-white text-xs font-bold rounded-full animate-pulse shadow-lg">
                                                LIVE NOW
                                              </span>
                                            )}
                                          </div>
                                          <h4 className="font-bold text-gray-900 mb-1 text-lg group-hover:text-[#19aaba] transition-colors">
                                            {event.title}
                                          </h4>
                                          ...
                                          {/* Event Type Badge */}
                                          <div className="mt-2">
                                            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${getTypeColor(event.type)} bg-opacity-10`}>
